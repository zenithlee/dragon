...
half4 CalculateLight (v2f i)
{
	...
	fixed numShades = nspec.a*20;
	half diff = max (0, dot (lightDir, normal));
	diff = floor( diff * numShades);
	diff = min( diff, numShades-1) / numShades;

	fixed spec = max(0.0, dot(normal, normalize (lightDir - normalize(wpos-_WorldSpaceCameraPos))));
	spec = step(0.5,pow(spec, (1-nspec.a)*128));
	
	half4 res;
	res.xyz = ((_LightColor.rgb * diff) + (_LightColor.rgb / numShades))*atten;
	res.w = spec*atten;
