* lfsmerge2007_1.do
* Merge all modules of South Africa Labour Force Survey for 2007_1
...
local datafolder="C:\Users\admin\Desktop\Andy\DataFirst\main project\LFSdata\LFS2007\LFS2007_1"

use "`datafolder'\LFS 2007_1 Person.dta"
	gen str10 dataset="2007p1"
	gen wave=15
...
duplicates report personid
	...
	* getting a useful EA variable, I am not sure if this is right but does produce numbers of hh per ea pretty similar to pre 2006 surveys..
	drop psu
		sort uqnr
...
destring hhidsub1, gen(psu)
	...
	save "`datafolder'\lfs2007p1temp.dta", replace

	use "`datafolder'\lfs2007_1.cewgt.dta", replace


	merge 1:m uqnr using "`datafolder'\lfs2007p1temp.dta"
	tab _merge
	rename _merge mergeh1
	list  uqnr if mergeh1==2
	*5 indivs from one hh not matched.. So dont have CE weights..
	sort personid
save "`datafolder'\lfs2007p1.dta", replace


use "`datafolder'\LFS 2007_1 Worker.dta"
	*again, make lower case and make uqnr a double
	renvars, lower
	destring uqnr, replace
	gen str10 dataset="2007w1"
	gen wave=15
	*included psu variable unhelpful, so drop
	drop psu
********************************************************
duplicates report uqnr personnr
...
duplicates report
duplicates report personid
	save "`datafolder'\lfs2007w1.dta", replace

merge personid using "`datafolder'\lfs2007p1.dta"
...
order hhid personid
compress
label data "South Africa Labour Force Survey 2007_1 merged"
quietly do "`mergefolder'\LFSrename2007_1.do"
...
sort personid
numlabel, add
save "`datafolder'\lfs2007_1.dta", replace
save "`mergefolder'\lfs2007_1.dta", replace
