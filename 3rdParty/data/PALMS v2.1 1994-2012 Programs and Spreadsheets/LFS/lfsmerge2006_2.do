* lfsmerge2006_2.do
* Merge all modules of South Africa Labour Force Survey for 2006_2
...
local datafolder="C:\Users\admin\Desktop\Andy\DataFirst\main project\LFSdata\LFS2006\LFS2006_2"

use "`datafolder'\LFS_2 2006 Person.dta"
	gen str10 dataset="2006p2"
	gen wave=14
	
	destring b_4night , replace
...
duplicates report personid
	save "`datafolder'\lfs2006p2temp.dta", replace
	
	use "`datafolder'\lfs2006_2.cewgt.dta", replace
	
	merge 1:m uqnr using "`datafolder'\lfs2006p2temp.dta"
	tab _merge
	rename _merge mergeh1
	sort personid
save "`datafolder'\lfs2006p2.dta", replace

	
use "`datafolder'\LFS_2 2006 Worker.dta"
	gen str10 dataset="2006w2"
	gen wave=14
	*psu variable unhelpful, proper one created above, so drop
	drop psu
********************************************************
* pull out constant 0 in 6th column of hhid to reduce size and allow creation of personid *
	gen str20 tempid=string(uqnr, "%16.0f" )
...
duplicates report
duplicates report personid
	save "`datafolder'\lfs2006w2.dta", replace

merge personid using "`datafolder'\lfs2006p2.dta"
...
order hhid personid
compress
label data "South Africa Labour Force Survey 2006_2 merged"
quietly do "`mergefolder'\LFSrename2006_2.do"
...
sort personid
numlabel, add
save "`datafolder'\lfs2006_2.dta", replace
save "`mergefolder'\lfs2006_2.dta", replace
